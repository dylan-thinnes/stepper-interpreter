...
matchPat (LitP pat) (LitE exp)
    | pat == exp = Success []
    | otherwise = Failure $ pure $ concat ["Literals unequal: pattern ", show pat, ", expression ", show exp, "."]
matchPat (VarP name) exp = Success [(name, exp)]
matchPat (TupP pats) (TupE exps)
  | length pats /= length exps = Failure $ pure "Tuples are of different lengths."
  | any isNothing exps = Failure $ pure "Cannot pattern match over partially applied tuples."
...
matchPat (ConP patConName pats) exp =
  ...
  case f of
      ConE expConName
        | expConName /= patConName -> Failure $ pure "Pattern and expression have different constructor names."
        | length pats /= length args -> Failure $ pure "Pattern and expression have different number of args to constructors - is the expression constructor partially applied?"
        | otherwise -> fold $ zipWith matchPat pats args
      _ -> Failure $ pure "Expression is not a data construction."
...
matchPat (ListP pats) (ListE exps)
  | length pats /= length exps = Failure $ pure "List pattern and list expression have different lengths."
...
matchPat _ _ = error "Pattern and expression do not match." -- TODO: Is this exhaustive enough?
